...
def _build_contract(
    ...
    cs = SPIService.compute_checksum(body)
    if corrupt_checksum:
...
def initialized_controller(mock_spi):
    """ESPDataController backed by a mocked SPIService with contract exchanged."""
    with patch("control.services.SPIService.spidev.SpiDev") as mock_cls:
        ...
        ctrl = ESPDataController(SPIService(COMM_DETAILS))
        ctrl.initialize()
        ctrl.read_contract()
    return ctrl, mock_spi


# ── SPIService.compute_checksum ──────────────────────────────────────────────────────────────────────────

class TestXorChecksum:
    def test_empty(self):
        assert SPIService.compute_checksum(b"") == 0

    def test_single_byte(self):
        assert SPIService.compute_checksum(bytes([0xAB])) == 0xAB

    def test_known_value(self):
        assert SPIService.compute_checksum(bytes([0x01, 0x02, 0x03])) == 0x00

    def test_self_cancel(self):
        assert SPIService.compute_checksum(bytes([0xFF, 0xFF])) == 0x00
...
class TestContractParsing:
    ...
    def test_bad_checksum_falls_back_to_defaults(self, mock_spi):
        """Bad contract checksum → SensorReadError caught → retries → defaults."""
        with patch("control.services.SPIService.spidev.SpiDev") as mock_cls:
            mock_cls.return_value = mock_spi
            mock_spi.xfer2.return_value = list(_build_contract(corrupt_checksum=True))
            ctrl = ESPDataController(SPIService(COMM_DETAILS))
            ctrl.initialize()
            ctrl.read_contract(max_attempts=1)
        assert ctrl._fields == [
            "yaw", "pitch", "roll",
            "qx", "qy", "qz", "qw",
            "gx", "gy", "gz",
